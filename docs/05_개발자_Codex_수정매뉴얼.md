...
11. 기본 백업을 제거하거나 우회하지 않는다. `--no-backup`은 사용자가 디스크 영향을 알고 명시적으로 선택할 때만 쓴다.
...
Codex의 기본 순서를 바꾸려면 `AGENTS.md`를 수정한다. 사용자용 설명은 `docs/01_사용자_매뉴얼.md`, 복사·붙여넣기용 초기 설정 지시는 `docs/08_Codex_기본세팅_프롬프트.md`, 실제 재현 순서는 `docs/02_전체_재현_절차.md`, 실패 기준은 `docs/03_HWPX_무결성_및_장애대응.md`를 함께 갱신한다.

초기 설정 프롬프트를 수정할 때는 다음 항목을 빠뜨리지 않는다.

- 저장소 루트 확인
- 운영체제와 Python 버전 확인
- 플랫폼별 설치 스크립트 실행
- 전체 자동 테스트 통과 확인
- 샘플 파이프라인과 `passed: true` 확인
- `사용자작업` 입력 폴더 생성
- 원본과 실제 사용자 파일을 기본 세팅 중에는 건드리지 않는 규칙
- 실패를 성공으로 보고하지 않는 규칙
